    ...
    dbc.Row(
        [  # vierte Zeile
            dbc.Col([
                ...
                dbc.Row(COL_Joystick),
                dbc.Row(COL_Modi_start),
                dbc.Row(COL_Modi_openCV),
                dbc.Row(COL_Modi_cnn),
                dbc.Row(COL_Modi_stop)
            ],
...
@app.callback(
    ...
    Input("btn_openCV", "n_clicks"),
    Input("btn_cnn", "n_clicks"),
    Input("btn_stop", "n_clicks")
)

def buttons_click(btn_start, btn_openCV, btn_cnn, btn_stop):
    cam = CamCar()
    global modus
    """Buttons Fahrmodus verarbeiten.
    Returns:
        
    """
    changed_id = [p["prop_id"] for p in callback_context.triggered][0]
    if "btn_start" in changed_id:
        modus = cam.drive(35, 1)
    if "btn_openCV" in changed_id:
        modus = cam.modus_opencv()
    if "btn_cnn" in changed_id:
        modus = cam.test_cnn()
